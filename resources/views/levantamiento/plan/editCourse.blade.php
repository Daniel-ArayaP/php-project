...
<div class="container-fluid">
    <h2>Levantamiento de Requisitos - Editar Materia</h2>
    <hr />
    <br />
        ...
        <div id="pes" class="tab-pane fade in active">
            <div class="panel panel-green">
                <div class="panel-body">
                    <form id="form" method="post" action="{{ route('updateContenidoCarrera',['idPlan' => $plan->id_plan_estudios, 'idCourse' => $course->id_contenido_carreras]) }}">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group">
                            <label>Código</label>
                            <input id="id" name="id" required="required" maxlength="7"  class="form-control" value="{{$course->id_contenido_carreras}}" disabled="disabled">
                        </div>
                        <div class="form-group">
                            <label>Nombre de la materia:</label>
                            <input id="name" name="name" required="required" maxlength="150"  class="form-control" value="{{$course->nombre_contenido_carreras}}">
                        </div>
                        <div class="form-group">
                            <label>Créditos:</label>
                            <input id="credits" name="credits" required="required" min="1" max="9" class="form-control" type="number" value="{{$course->creditos_contenido_carreras}}">
                        </div>
                        <div class="form-group">
                        ...
                        <table align="center" width="60%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody>
                                @foreach ($course->correquisitos as $correquisito)

                                <tr>
                                    <td>{{$correquisito->id_contenido_carreras}}</td>
                                    <td>{{$correquisito->nombre_contenido_carreras}}</td>
                                    <td><button type="button" class="btn btn-danger">Eliminar Correquisito</button></td>
                                </tr>
                                @endforeach             
                            </tbody>
                        </table>
            ...
            <label for="id_label_single">
                ...
                <select class="js-example-basic-single" id='corequisite' name="corequisite" style="width:100%" >
                    @foreach ($plan->courses as $planCourse)
                        @if($planCourse->details->id_contenido_carreras!=$course->id_contenido_carreras)    
